...
public class PaymentController implements Initializable {
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty) {
                    ...
                    if (isPaid) {
                        ...
                        Label paidLabel = new Label("PayÃ©");
    ...
    private void setupPaymentTable() {
        ...
        // Set up the actions column with only the edit button
    ...
    private void handleSearch(ActionEvent event) {
        ...
        if (completedAppointments.isEmpty()) {
            showAlert(Alert.AlertType.INFORMATION, "Aucun RÃ©sultat",
                    "Aucun rendez-vous terminÃ© trouvÃ© pour ce patient aujourd'hui.");
    ...
    private void showPaymentDialog(Appointment appointment) {
        ...
        Label methodLabel = new Label("MÃ©thode de Paiement:");
        ComboBox<String> methodComboBox = new ComboBox<>();
        methodComboBox.getItems().addAll("Cash", "Credit Card", "Insurance");
        methodComboBox.setValue("Cash");
        ...
        result.ifPresent(payment -> {
            ...
            if (success) {
                showAlert(Alert.AlertType.INFORMATION, "SuccÃ¨s", "Paiement enregistrÃ© avec succÃ¨s.");
                loadPaymentHistory(); // Refresh payment history
                appointmentTable.refresh(); // Refresh appointment table to update paid status
            } else {
                showAlert(Alert.AlertType.ERROR, "Erreur", "Ã‰chec de l'enregistrement du paiement.");
            }
        });
    }
    ...
    private void showEditPaymentDialog(Payment payment) {
        ...
        result.ifPresent(updatedPayment -> {
            ...
            if (success) {
                showAlert(Alert.AlertType.INFORMATION, "SuccÃ¨s", "Paiement mis Ã  jour avec succÃ¨s.");
                loadPaymentHistory(); // Refresh payment history
            } else {
                showAlert(Alert.AlertType.ERROR, "Erreur", "Ã‰chec de la mise Ã  jour du paiement.");
            }
        });
    }
    ...
    private void navigateTo(String fxmlPath) {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur de Navigation",
                    "Impossible de charger la page demandÃ©e: " + e.getMessage());
        }
    }
}